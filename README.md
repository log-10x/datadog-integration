# log10x Datadog integration

A Datadog Agent check for the [10x Engine](https://www.log10x.com). It reads the engine's
Prometheus metrics endpoint and submits per-pattern log volume and byte counters to Datadog.

This is the source of the integration submitted to Datadog's catalogue as
[DataDog/integrations-extras#3157](https://github.com/DataDog/integrations-extras/pull/3157).
Until that is merged and the wheel is published by Datadog, install it from the
[release](https://github.com/log-10x/datadog-integration/releases/latest):

```bash
curl -LO https://github.com/log-10x/datadog-integration/releases/download/v1.0.0/datadog_log10x-1.0.0-py3-none-any.whl
sudo -u dd-agent datadog-agent integration install -w ./datadog_log10x-1.0.0-py3-none-any.whl
```

For a containerised Agent, add the wheel in a Dockerfile layer:

```dockerfile
FROM datadog/agent:7
COPY datadog_log10x-1.0.0-py3-none-any.whl /tmp/
